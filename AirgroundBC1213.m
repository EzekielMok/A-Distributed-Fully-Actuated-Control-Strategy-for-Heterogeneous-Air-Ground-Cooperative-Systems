...
for j = sim.t
    ...
    if j>=time_start_scaleup && j<=time_start_scaleup+time_span_scaledown
        % agent 3
        a=Accel_slow_fast(time_start_scaleup, time_span_scaledown, j,1);
        % agent 3
        b=Accel_slow_fast(time_start_scaleup, time_span_scaledown, j, 1);
        a_interg = a_interg + a * sim.ts;
        aa_interg = aa_interg + a_interg * sim.ts;
        b_interg = b_interg + b * sim.ts;
        bb_interg = bb_interg + b_interg * sim.ts;
...
hold on; box on;
grid off
set(gcf, 'Position', [100, 100, 1.2*800, 1.2*600]);
...
[satellite_1,combinedobject1] = carplot(gca,agcolor1);
set(combinedobject1,'matrix',...
    makehgtform('translate', [agent1.record_x(index_star),agent1.record_y(index_star)-2,agent1.record_z(index_star)+2])*makehgtform('zrotate',agent1.record_psi(index_star)));
...
translation1 = makehgtform('translate', [agent3.record_x(index_star),agent3.record_y(index_star),agent3.record_z(index_star)]);
    ...
    translation1*rotation13*rotation12*rotation11);
 [satellite_5,combinedobject5] = carplot(gca,agcolor1);
...
translation1 = makehgtform('translate', [agent2.record_x(index_star)+2.2,agent2.record_y(index_star)-2.2,agent2.record_z(index_star)+4]);
    rotation11 = makehgtform('xrotate',(pi/180)*(0));
    rotation12 = makehgtform('yrotate',(pi/180)*(0));
    rotation13 = makehgtform('zrotate',agent2.record_psi(index_star));
...
translation1 = makehgtform('translate', [agent3.record_x(index_star)-2.2,agent3.record_y(index_star)+2.2,agent3.record_z(index_star)+4]);
    rotation11 = makehgtform('xrotate',(pi/180)*(0));
    rotation12 = makehgtform('yrotate',(pi/180)*(0));
    rotation13 = makehgtform('zrotate',agent3.record_psi(index_star));
    set(combinedobject7,'matrix',...
    translation1*rotation13*rotation12*rotation11);
 [satellite_8,combinedobject8] = carplot(gca,agcolor1);
set(combinedobject8,'matrix',...
    makehgtform('translate', [agent1.record_x(index_star),agent1.record_y(index_star),agent1.record_z(index_star)+2])*makehgtform('zrotate',agent1.record_psi(index_star)));
line([agent1.record_x(index_star),agent2.record_x(index_star)],[agent1.record_y(index_star), ...
    agent2.record_y(index_star)],[agent1.record_z(index_star), ...
    agent2.record_z(index_star)],'Color',line_color,'LineStyle',line_styte, LineWidth=line_w)
line([agent1.record_x(index_star),agent3.record_x(index_star)],[agent1.record_y(index_star), ...
...
legend('Virtual leader','UGV','UAV 1','UAV2','Location','northEast','Orientation','Vertical');
function [satellite_1,combinedobject] = carplot(hg,cor)
ox1=0;
oy1=0;
body_length = 4;
body_color = cor; % 卫星主体颜色
yscale=1.8;
r_p=1.5;
x_p=-2;
y_p=-2;
...
hgt=1;
col='k';
[x_x2,y_y2,z_z2] = cylinder(1);
[box_a,box_b]=size(x_x2);
x_x1=x_x2;
y_y1=y_y2;
z_z1=z_z2;
theta1=pi/2;
...
for i=1:1:box_a
    for j=1:1:box_b
        temp=T_tran*[r_p*x_x2(i,j),r_p*y_y2(i,j),hgt*z_z2(i,j)]';
        x_x1(i,j)= temp(1);
        y_y1(i,j)= temp(2);
        z_z1(i,j)= temp(3);
    end
...
to = linspace(0, 2*pi);
xp = x_p+r_p*cos(to);
yp = y_p+zeros(1,length(to));
zp = z_p+r_p*sin(to);
satellite_1(1)= surf(x_p+x_x1,y_p+y_y1,z_p+z_z1,'FaceColor',col,'EdgeColor','none'); 
satellite_1(2)  = patch(xp,yp,  zp,col,'EdgeColor','w');
satellite_1(3)  = patch(xp,yp-hgt,  zp,col,'EdgeColor','w');
satellite_1(4)= surf(x_p+x_x1+4,y_p+y_y1,z_p+z_z1,'FaceColor',col,'EdgeColor','none'); 
satellite_1(5)  = patch(xp+4,yp,  zp,col,'EdgeColor','w');
satellite_1(6)  = patch(xp+4,yp-hgt,  zp,col,'EdgeColor','w');
satellite_1(7)= surf(x_p+x_x1,y_p+y_y1+5,z_p+z_z1,'FaceColor',col,'EdgeColor','none'); 
satellite_1(8)  = patch(xp,yp+5,  zp,col,'EdgeColor','w');
satellite_1(9)  = patch(xp,yp-hgt+5,  zp,col,'EdgeColor','w');
satellite_1(10)=surf(x_p+x_x1+4,y_p+y_y1+5,z_p+z_z1,'FaceColor',col,'EdgeColor','none'); 
satellite_1(11)  = patch(xp+4,yp+5,  zp,col,'EdgeColor','w');
satellite_1(12)  = patch(xp+4,yp-hgt+5,  zp,col,'EdgeColor','w');
satellite_1(13)=patch(ox1+yscale*[-1, -1, 1, 1]*body_length/2, oy1+[-1, 1, 1, -1]*body_length/2, [-1, -1, -1, -1]*body_length/2, body_color,'Linewidth',1);
satellite_1(14)=patch(ox1+yscale*[-1, -1, 1, 1]*body_length/2, oy1+[-1, 1, 1, -1]*body_length/2, [1, 1, 1, 1]*body_length/2, body_color,'Linewidth',1);
satellite_1(15)=patch(ox1+yscale*[-1, 1, 1, -1]*body_length/2, oy1+[-1, -1, -1, -1]*body_length/2, [-1, -1, 1, 1]*body_length/2, body_color,'Linewidth',1);
satellite_1(16)=patch(ox1+yscale*[-1, 1, 1, -1]*body_length/2, oy1+[1, 1, 1, 1]*body_length/2, [-1, -1, 1, 1]*body_length/2, body_color,'Linewidth',1);
satellite_1(17)=patch(ox1+yscale*[1, 1, 1, 1]*body_length/2, oy1+[-1, 1, 1, -1]*body_length/2, [-1, -1, 1, 1]*body_length/2, body_color,'Linewidth',1);
satellite_1(18)=patch(ox1+yscale*[-1, -1, -1, -1]*body_length/2, oy1+[-1, 1, 1, -1]*body_length/2, [-1, -1, 1, 1]*body_length/2, body_color,'Linewidth',1);
combinedobject = hgtransform('parent',hg );
set(satellite_1,'parent',combinedobject)
end
